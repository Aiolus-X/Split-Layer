...
#def gaussianencoding(coord):


def positionalencoding(coord, fre_dim): #input N*2
    x, y = coord[:,0].unsqueeze(1), coord[:,1].unsqueeze(1) # N*1
    for i in range(fre_dim):
        coord=torch.cat((coord, torch.sin(2**i*np.pi*x), torch.sin(2**i*np.pi*y), torch.cos(2**i*np.pi*x), torch.cos(2**i*np.pi*y)), dim=1)
    return coord
...
def to_coordinates_and_features(img):
    ...
    #coord=positionalencoding(coordinates, 10) #(H*W) * 42
    #print('Complete Positional Encoding')
