...
class FinerLayer(nn.Module):
    ...
    def init_weights(self):
        with torch.no_grad():
            if self.is_first:
                self.linear1.weight.uniform_(-1 / self.in_features, 
                                             1 / self.in_features) 
                self.linear2.weight.uniform_(-1 / self.in_features,
                                             1 / self.in_features)
            else:
                self.linear1.weight.uniform_(-np.sqrt(6 / self.in_features) / self.omega_0,
                                             np.sqrt(6 / self.in_features) / self.omega_0)

                self.linear2.weight.uniform_(-np.sqrt(6 / self.in_features) / self.omega_0,
                                             np.sqrt(6 / self.in_features) / self.omega_0)
    
    def init_first_bias(self):
        with torch.no_grad():
            if self.is_first:
                self.linear1.bias.uniform_(-self.first_bias_scale, self.first_bias_scale)
                self.linear2.bias.uniform_(-self.first_bias_scale, self.first_bias_scale)
...
# print('init fbs', self.first_bias_scale)
    ...
    def forward(self, input):
        ...
        scale1 = self.generate_scale(x1)
        x2 = self.linear2(input)
        scale2 = self.generate_scale(x2)
        out = torch.sin(self.omega_0 * scale1 * x1) * torch.sin(self.omega_0 * scale2 * x2)
        return out
